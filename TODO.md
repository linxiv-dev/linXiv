...
## Deferred
- [ ] **`pdf_url` rename in `SearchResultOut`** — `pdf_url` is misnamed for OpenAlex results, which return a DOI landing page or OpenAlex work URL, not a PDF URL. Fixing it requires updating the frontend `SearchResult` TypeScript interface. See ADR 0011.
